...
**补测收口（2026-09-15）**：h/d 均已补测——h 中间态覆盖率 0% → 判全量重写（非句级修补）；d 5/5 失败原句在单源改写下全部判 FAIL。记录见 `_plans/write-introduction-batch7-acceptance-2026-09-15.md`（commit 268081c），§7 通过线的两项必过项（d、h）就此闭合。

### D-08｜P2-8「SKILL.md ≤90 行」判据偏离注记（2026-09-15，执行段）

- **原规格**：P2-8 完成判据含「SKILL.md ≤90 行且判据可'是/否'回答；同义整句复述 grep 归零」。
- **实测**：四协议落地提交（b298ebd）时即 109 行；Batch 2–5 共享接线后为 111 行——增量来自 Phase 3 G1 底本指针、Phase 4 措辞润色的 key-line/跨节单一源指针等强制接线（D-06「保留并接线」、共享 pass-contract 收口）。
- **处置**：判据另两个成分（完成判据可布尔回答、同义整句复述 grep 归零）已达标；行数成分判为**可辩护偏离，不回改**——为凑行数删指针会破坏 Batch 2–5 的单一源纪律与渐进披露。后续若 SKILL.md 再膨胀，按渐进披露原则把成段内容下沉 references，不以「回到 ≤90」为目标。
