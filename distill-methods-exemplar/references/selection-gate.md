...
在进入 Phase 1 深读前，用 `write-methods/corpus/_evidence_registry.yaml` 中该设计类型的 `validation_history` 判断**这篇论文值不值得深蒸馏、优先级多高**。批评由 Claude 在 write-methods 会话中自动登记，也可用 `_update_registry.py --record-critique` 批量补登（见 `phase-4-validation-writeback.md` 批评登记）。
