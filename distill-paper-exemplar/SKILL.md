...
| 层 | 动作 | 产物 / 门禁 |
|---|---|---|
| L0 | `paper-import` 全文 MD → `scripts/preprocess_l0.py`（剥 base64、物化切片、slice-check 兜底、`--keep-sentences`） | `<citekey>.pdm/` + PDM 骨架（status: manifest） |
| L1 | 四节蒸馏子代理（2+2 波次，契约见 `references/l1-subagent-protocol.md`）→ corpus_precheck plan（fail-fast）→ gate ① 批量呈审 → corpus_writeback（幂等，含 create_new_file）→ verify_writeback 终验 | write-* 语料 |
| L2 | 主循环按 `references/cross-section-coherence.md` 交叉校验四节 identity | ok / flagged，只标记不擅改 |
| L3 | distill-story-exemplar ← PDM 切片 + verified 分节蒸馏（与残项同步 pass 并行） | story 卡（gate ②）+ validate/build |
| L4 | feedback 归拢 → `--clean`（本篇工作目录）→ `--sweep`（跨篇中间产物） | 三路输出落点报告 |

各层操作细节与判据见「工作流」（唯一权威陈述，图不重复）。
...
1. **L0 获取与分解**。PDF → `paper-import`（80 页内 OvisOCR2，超长/图书 MinerU）；已是 MD
   则直接用。**随后必跑** `py scripts/preprocess_l0.py <全文MD>`（确定性脚本，不经 LLM）：
   ...
   **句子库存例外（P1a）**：`preprocess_l0.py <MD> --keep-sentences`
...
2. **L1 分节蒸馏分发（子代理）**。按用户范围（默认 4 节全跑）以 **2+2 波次并行**分发
   ...
   plan 由 `corpus_precheck.py` 产出（确定性：选带/查重/锚点；fail-fast——缺 block_text、
...
- 同一篇论文被多次蒸馏时，以新 PDM 为准；已有旧 PDM 则续跑而非重造（见 schema 状态机）。
- **单窗口纪律**：同一 PDM 同一时间只在一个会话跑——L0 创建 `<citekey>.pdm/LOCK`
  （12h 内拒绝二次启动，陈旧锁 `--force`，完成后 `--unlock`/`--clean`）。双窗口会把
  L1–L3 整链路跑两遍，是单次运行最大的浪费源。
