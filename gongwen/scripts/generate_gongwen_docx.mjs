...
function writeDocx(output, documentXml, pageNumberMode, format) {
  ...
  // Windows 打包回退：无 Info-ZIP 时用系统自带 bsdtar（System32\\tar.exe）。
  // 显式枚举顶层条目，避免 bsdtar 的 "./" 前缀破坏 OOXML 严格读取器。
  const packAttempts = [];
  if (process.platform === "win32") {
    const winTar = path.join(process.env.SystemRoot || process.env.windir || "C:\\Windows", "System32", "tar.exe");
    if (fs.existsSync(winTar)) {
      packAttempts.push({ cmd: winTar, args: ["-cf", output, "--format", "zip", ...fs.readdirSync(dir)] });
    }
  }
  packAttempts.push({ cmd: "zip", args: ["-qr", output, "."] });
  let packError;
  for (const attempt of packAttempts) {
    try {
      execFileSync(attempt.cmd, attempt.args, { cwd: dir });
      packError = null;
      break;
    } catch (err) {
      packError = err;
