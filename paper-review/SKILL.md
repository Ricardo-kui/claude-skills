...
- Introduction 的语言层审查路由到 `intro-review`，其字段契约见 `../_shared/pass-contract.md`；路由时一并指向该契约，语言层无契约空档。
...
- 双层各司其职：叙事层诊断结构与故事；实质层（toc-review 规则）产出带证据引文的弱点记录。语言逐字润色留给下游 review skills；Introduction 的语言层审查由 `intro-review` 按 `../_shared/pass-contract.md` 承接（字段契约见该文件）。
