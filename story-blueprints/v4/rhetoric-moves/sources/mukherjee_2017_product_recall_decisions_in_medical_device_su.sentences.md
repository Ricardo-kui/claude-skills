...
citekey: "mukherjee_2017_product_recall_decisions_in_medical_device_su"
source_md: "D:\Onedrive\Obsidian Vault\文献笔记库\01 导入\论文导入\Mukherjee等-2017-Product Recall Decisions in Medical Device Supply Chains A Big Data Analytic Approach to Evaluating Judgment Bias.md"
created: 2026-09-20
...
# mukherjee_2017_product_recall_decisions_in_medical_device_su 句子库存
...
Hence, the questions that serve as the motivation for this study are: (i) Do firms systematically exhibit a judgment bias in interpreting user-generated market feedback on adverse events related to their products?
<!-- para 7 -->
...
Second, decision makers can miss out on
<!-- para 7 -->
*Figure 1.
Operationalization of Signal Detection Theory Depicting the Effect of Decision Process (Decision Threshold, $X_{c}$)*
<!-- para 9 -->
detecting a credible recall signal and do not recall a device when the device should have been recalled.
...
<!-- para 10 -->
Let $C_{s}$ be the sunk cost of a false alarm and $C_{o}$ be the opportunity cost of a miss.
...
<!-- para 15 -->
**2.1.1.
Signal Detection and Judgment Bias: Under-Reaction and Over-Reaction.** Both the false alarm rate and the miss rate are dependent on the choice of a decision threshold by decision makers.
...
Judgment Bias : c
<!-- para 17 -->
$$ =\frac{False Alarm Rate\ (FAR)-Miss Rate\ (MR)}{2}. \tag{3} $$
<!-- para 18 -->
...
<!-- para 19 -->
**2.1.2.
Consistent Estimation of Judgment Bias from Data.** The critical factor that determines the optimal decision threshold ( $X_c^o$) is $\lambda$, the natural logarithm of the ratio of the costs associated with false alarm rate and miss rate (Equation 2).
...
<!-- para 36 -->
HYPOTHESIS 3 (H3).
Ceteris paribus, firm size is positively (negatively) associated with under-reaction (overreaction) bias.
<!-- para 37 -->
...
The constituent hypotheses of the framework depicted in Figure 2 relate the signal characteristics – captured in terms of the constructs, noise-to-signal ratio and severity of adverse events; and the situated context of decision makers – captured in  terms of the constructs, firm size and product portfolio – to over-reaction and under-reaction biases.
<!-- para 41 -->
*Figure 2.
The Measurement Level Framework for Evaluating Judgment Bias in Product Recall Decisions*
...
We follow the iteratively following steps in estimating judgment bias:
<!-- para 8 -->
Step 1: Build a recall prediction model of device recall with data from MAUDE and other predictor databases with train data from a 80:20 split of sample into train and test set.
<!-- para 9 -->
...
<!-- para 13 -->
**3.2.1.
Data Organization and Variable Generation.** Data organization represents an important step toward building a predictive model.
...
<!-- para 15 -->
**3.2.2.
Response Variables for Product Recall Prediction.** The unit of analysis for the predictive modeling and judgment bias measurement steps is a medical device recall.
...
<!-- para 16 -->
**3.2.3.
Predictor Variables for Product Recall Prediction.** The primary predictor variable for recall likelihood prediction model and judgment bias estimation is the kernel density of adverse events given by $f_t = \left(\frac{1}{N h}\right) \sum_{i=1}^N I_i \times K\left(\frac{t - T_i}{h}\right)$.
...
<!-- para 18 -->
**3.2.4.
Prediction Modeling Framework.** As already stated, the prediction modeling step is required to estimate judgment bias.
...
When the adverse event reports are supplemented by system level covariates like design covariates, supply chain covariates, and manufacturing covariates, the precision of prediction is likely to improve further.
...
<!-- para 19 -->
*Figure 3.
Predictive Model Building for the Estimation of Judgment Bias in Product Recall Decisions*
<!-- para 21 -->
**3.2.5.
Variable Selection for Prediction of Recalls.** Variable selection is a key step in predictive model building (Shmueli 2010) to ensure model parsimony and to avoid over-fitting.
...
Since the measure is relative, the prediction parameters have limited effect on the bias measure as  long as the prediction parameters are kept invariant within a product (identified by three letter code issued by FDA for each product).
...
Figure 5 shows that, in general, firms have a much higher likelihood of under-reaction bias as compared to over-reaction bias, both in terms of the frequency (measured by kernel density of the bias) with which firms exhibit under-reaction bias as against over-reaction bias and in terms of the extent of under-reaction bias (measured in absolute numerical scale of the bias measure).
<!-- para 29 -->
*Figure 4.
Receiver Operating Characteristics (ROC) Curve for the Random Forest Model (Equation 6)*
<!-- para 31 -->
*Figure 5.
Distribution of the Judgment Bias in the Study Sample*
<!-- para 34 -->
...
<!-- para 41 -->
**3.6.1.
Product-related Control Variables.** Regulation Type PMA is a categorical variable coded as one for pre-market approval (PMA) corresponding to  approval routed for new medical devices and zero corresponding to approval route for new versions of existing medical devices.
<!-- para 42 -->
...
<!-- para 46 -->
**3.6.2.
Manufacturer-related Control Variables.** CAGR Fixed Asset is a measure of the compound annual growth rate (CAGR) of the gross fixed assets of a firm excluding assets acquired through mergers or acquisitions but including assets diluted.
...
<!-- para 61 -->
$$
\begin{aligned}log\!\left(\frac{P(\text{BiasOR}_{ij}=1)}{1-P(\text{BiasOR}_{ij}=1)}\right)\\\qquad = \beta_{i0} + \beta_{1}(Noise-t-Signal\ Ratio) + \beta_{2}(Severity_{ij})\\+\beta_{3}(Product\ Age_{ij})\\+\beta_{4}(Product\ Class\ \Pi_{ij}) + \beta_{5}(Product\ Class\ \Pi_{ij})\\+\beta(\text{Regulation Type}\ PMA_{ij})\\+\sum_{i=7}^{26} \beta_{i}(Usage\ Class_{ij})\\+\beta_{12}(Noise-to-Signal\ Ratio_{ij}) \times Severity_{ij})\\+\beta_{27}(Firm\ Size_{i}) + \beta_{28}(Product\ Portfolio\ Index_{i})\\+\beta_{29}(CAGR\ Fixed\ Assets_{i})\\+\beta_{30}(Mergers\ Acquisitions_{i}) + \beta_{31}(Foreign_{i})\\+\beta_{32}(CAGR\ Margin_{i})\\+\beta_{32}(Inventory\ Turnover_{i}) + \beta_{33}(Competition_{j})\\+\eta_{i} + \epsilon_{ij}\end{aligned}
$$
<!-- para 62 -->
(9)
...
<!-- para 7 -->
*Table 2.
Model Estimation Results for Evaluating Judgment Bias in Medical Device Recall Decisions (Response Variable: Continuous Measure of Bias)*
<!-- para 8 -->
<table><tr><td rowspan="2"></td><td colspan="5">Columns</td></tr><tr><td>(1)</td><td>(2)</td><td>(3)</td><td>(4)</td><td>(5)</td></tr><tr><td>(Intercept)</td><td>0.4931***<br>(0.0306)</td><td>0.8105***<br>(0.1789)</td><td>0.8077***<br>(0.1794)</td><td>0.8055***<br>(0.1775)</td><td>1.0363***<br>(0.1833)</td></tr><tr><td>Product level variables</td><td></td><td></td><td></td><td></td><td></td></tr><tr><td>Noise-to-Signal Ratio</td><td>-0.3978***<br>(0.0855)</td><td>-0.6111***<br>(0.0958)</td><td>-0.6121***<br>(0.0961)</td><td>-0.5861***<br>(0.0964)</td><td>-0.5709***<br>(0.0941)</td></tr><tr><td>Severity</td><td>1.3005***<br>(0.3301)</td><td>1.3762***<br>(0.3298)</td><td>1.1157***<br>(0.3346)</td><td>0.7296*</td><td>0.4649*</td><td>(0.2332)</td></tr><tr><td>Product Age</td><td></td><td>-0.3227*</td><td>-0.3197*</td><td>-0.2882*</td><td>-0.4311*</td><td></td></tr><tr><td></td><td></td><td>(0.1347)</td><td>(0.1553)</td><td>(0.1451)</td><td>(0.1679)</td><td></td></tr><tr><td>Product Class2</td><td></td><td>0.0999+<br>(0.0575)</td><td>0.1001*</td><td>0.0916+<br>-0.0481</td><td>0.1389**<br>(0.0456)</td><td></td></tr><tr><td>Product Class3</td><td></td><td>0.2115***<br>(0.0595)</td><td>0.2121***<br>(0.0595)</td><td>0.2141***<br>(0.0603)</td><td>0.2384***<br>(0.0571)</td><td></td></tr><tr><td>Implant.1</td><td></td><td>0.0691*</td><td>0.0699*</td><td>0.0946*</td><td>0.0596+<br>(0.0357)</td><td></td></tr><tr><td>Regulation Type PMA</td><td></td><td>0.0793**<br>(0.0298)</td><td>0.0676*</td><td>0.0497+<br>(0.0304)</td><td>0.0239</td><td>(0.0392)</td></tr><tr><td>Usage Class</td><td></td><td>S</td><td>S</td><td>S</td><td>S</td><td></td></tr><tr><td>Noise-to-Signal X Severity</td><td></td><td></td><td>1.2454*</td><td>1.1253**<br>(0.4277)</td><td>0.9439*</td><td>(0.4189)</td></tr><tr><td>Firm level variables</td><td></td><td></td><td></td><td></td><td></td><td></td></tr><tr><td>Firm Size</td><td></td><td></td><td></td><td>-0.2194***<br>(0.0561)</td><td>-0.1265**<br>(0.0472)</td><td></td></tr><tr><td>Product Portfolio Index</td><td></td><td></td><td></td><td>-0.0895*</td><td>-0.0879*</td><td></td></tr><tr><td></td><td></td><td></td><td></td><td>(0.0376)</td><td>(0.0445)</td><td></td></tr><tr><td>CAGR Fixed Assets</td><td></td><td></td><td></td><td></td><td>-0.1741+<br>(0.1017)</td><td></td></tr><tr><td>CAGR Margin</td><td></td><td></td><td></td><td></td><td>0.1473**<br>(0.0561)</td><td></td></tr><tr><td>Inventory Turnover</td><td></td><td></td><td></td><td></td><td>0.0131</td><td></td></tr><tr><td>Mergers Acquisition Inv</td><td></td><td></td><td></td><td></td><td>-0.1335*</td><td></td></tr><tr><td></td><td></td><td></td><td></td><td></td><td>(0.0524)</td><td></td></tr><tr><td>RnD to Sales</td><td></td><td></td><td></td><td></td><td>-0.0538</td><td></td></tr><tr><td></td><td></td><td></td><td></td><td></td><td>(0.0694)</td><td></td></tr><tr><td>Foreign</td><td></td><td></td><td></td><td></td><td>0.0737*</td><td></td></tr><tr><td></td><td></td><td></td><td></td><td></td><td>(0.0337)</td><td></td></tr><tr><td>Competition</td><td></td><td></td><td></td><td></td><td>0.4191***<br>(0.0349)</td><td></td></tr><tr><td>F-Statistic</td><td>20.29***</td><td>8.0115***</td><td>8.9345***</td><td>7.8723***</td><td>7.3945***</td><td></td></tr><tr><td>AIC</td><td>1,717.7</td><td>1,615.4</td><td>1,601.3</td><td>1,555.4</td><td>1,501.1</td><td></td></tr><tr><td>n</td><td>1,348</td><td>1,348</td><td>1,348</td><td>1,348</td><td>1,348</td><td></td></tr></table>
<!-- para 9 -->
*Notes.
...
High severity  (With significant marginal effects only) of adverse event reports from users leads to higher likelihood for firms to become risk averse, and, in turn, there is likely to be more false alarms than misses (H2).
...
<!-- para 12 -->
*Figure 6.
Marginal Plots for Device Usage Classes and its Relationship to Judgment Bias (Over-Reaction and Under-Reaction)*
<!-- para 14 -->
...
Also, we have scaled all other variables for model estimation since  many of the variables such as Firm Size and Merger Acquisition Investment were measured at a very different scale compared to others.
...
The intended usage of a device is significant in determining how firms react to user-generated reports on adverse events related to devices.
...
<!-- para 25 -->
*Figure 8. $2 \times 2$ Matrix Summarizing the Study Results on the Impact of Signal Characteristics (Noise-to-Signal Ratio and Severity of Adverse Events) on Judgment Bias (Over-Reaction and Under-Reaction) in Medical Device Recall Decisions*
<!-- para 28 -->
...
Decision makers in firms either over-react or under-react.
...
<!-- para 6 -->
[^1]: http://www.nytimes.com/2014/09/15/business/regulator-slow-to-respond-to-deadly-vehicle-defects.html.
[^2]: http://www.bloomberg.com/news/articles/2014-03-12/better-data-would-have-started-gm-recall-sooner-nhtsa.
[^3]: The U.S. Government Accountability Office (GAO) is an independent, nonpartisan agency that works for Congress and serves as the “congressional watchdog.” (http://www.gao.gov/about/index.html).
