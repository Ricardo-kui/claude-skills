...
source_papers:
  ...
  - kalaignanam_kushwaha_eilert2013 (JM, 2013): "CPSC 2010 exponential trend + Evenflo 2007 infant seat instance + Toyota 2009 loss-of-life flagship — 严重性递进三段式"
  - chen_ganesan_liu2009 (JM, 2009): "serial vivid-instance enumeration (no numbers) + construct naming/definition anchor + 4-driver trend escalation; stats deferred to P2, stakes to P3"
  - thirumalai_sinha2011 (MS, 2011): "growth-face data first (robotics + FDA statutory definition + $90B/6%) → 'Although... sometimes also sources of significant risk' valence reversal → 'on one hand... bane of this industry' dual-face closure"
  - li_bapuji_talluri_singh_narayanan_2026_jscm (JSCM, 2026): "paired cross-industry disaster instances → vertical spillover definition (variant G)"
updated: 2026-08-06
created: 2026-05-18
...
### 变体 G：成对跨行业灾难实例化型（li_narayanan_2026_jscm 型）
...
### 变体 I：串行枚举 + 定义锚定 + 趋势升级型（chen2009 型）

**验证状态**: EMERGING（单篇来源；仅作 `section_variant`）

**功能节拍**: 异质实例串行枚举（无数字、纯画面）→ 现象命名 + 权威定义锚定 → 监管统计后置背书 → 四驱动趋势升级（"未来更频繁"）

**模板**:
> "Incidents of [negative phenomenon] are ubiquitous in [domain], ranging from [vivid instance 1], to [instance 2], to [instance 3], to [instance 4]. These well-publicized incidents are referred to as '[construct label]' ([defining citation]). They occur when [definitional boundary condition A, B, C, or D] ([citation]). Often, the consequence of [phenomenon] involves [focal event type], in which [event mechanics]. According to [regulatory body], [frequency statistic with year]. Moreover, [phenomenon/event] are likely to occur more often in the future because of [driver 1], [driver 2], [driver 3], and [driver 4] ([citation])."

**来源**: chen_ganesan_liu2009 (JM), P1-P2

**原文锚定**:
> "Incidents of negative publicity are ubiquitous in the marketplace, ranging from lead paint-contaminated toys, to faulty tires, to tainted pet food, to unhygienic food products. These well-publicized incidents are referred to as 'product-harm crises' (Dawar and Pillutla 2000). They occur when a firm's product fails to meet a mandatory safety standard, contains a defect that could cause substantial harm to consumers, creates an unreasonable risk of serious injury or death, or fails to comply with a voluntary standard adopted by the specific industry (Mullan 2004)."
>
> "According to the Consumer Product Safety Commission (CPSC), more than 400 consumer products were recalled in 2007 because of safety concerns. Moreover, product recalls are likely to occur more often in the future because of increased globalization of production, greater complexity of products, greater demand by consumers for product quality and safety, and closer monitoring by both firms and government agencies (Berman 1999)."

**关键特征**:
- **枚举建立 ubiquity 而非统计**：4 个跨品类实例（玩具/轮胎/宠物食品/餐饮）全靠画面感、无一个数字——读者凭公共记忆自行确认"确实常见"。与变体 A（监管统计开场）、变体 D（四重数字锚点）路径相反：先有画面，数字后置
- **"ranging from X, to Y, to Z, to W" 串行句式**：节奏型排比，实例间用逗号串联不换行，制造"数不胜数"的语感
- **枚举后立即命名 + 定义**："referred to as '[label]'... They occur when..." —— Hook 兼做构念界定，与变体 G（li_narayanan_2026_jscm）的定义收束同构，但本变体无第三方损失量化、实例不成对
- **统计后置 + 趋势升级**：监管数字（400+ recalls in 2007）在第二段才出现，且立刻被"likely to occur more often in the future because of [4 drivers]"升级——现象从"现在常见"变为"将更常见"，为 Stakes 的紧迫性铺路。四个驱动因素（globalization / product complexity / consumer demand / monitoring）覆盖供给侧与监管侧
- **能量级中低**：需要后接量化 Stakes 段（chen2009 P3 用 Merck 单日暴跌 $45.07→$33.00 + Topps 召回 21.7M 磅后破产双案例补足 stakes）

**适用**: 现象已被公众熟知但学术构念需要锚定（命名+定义）的研究；有权威定义可引（构念已有命名者）；能列举 ≥3 个跨行业真实事件；有文献支撑的前瞻性趋势论据。Incompleteness × Mechanism 组合适配；JM/JMR 等营销期刊首选

**禁忌**:
- 枚举实例必须真实且为读者耳熟能详——冷门案例无法触发公共记忆，枚举退化为罗列
- 趋势升级的驱动因素必须有文献支撑（Berman 1999），不可凭空预测"未来会更多"
- 定义 citation 不可省——否则构念命名变成作者生造
- 与变体 G 的边界：若能量化两个独立事件中**第三方**的损失（已发表 event-study 数字），用变体 G；本变体的实例是焦点行业本身的画面，不成对、不量化
- 不要堆砌超过 5 个实例——4 个是节奏感的上限，再多则读者疲劳

---

### 变体 J：权威统计 + 具体实例 + 旗舰案例三段式（kalaignanam2013 型）

**验证状态**: EMERGING（单篇来源；仅作 `section_variant`）

**功能节拍**: 宏观趋势统计（监管权威定调）→ 单一具体实例（画面感落地）→ 旗舰案例升级（severe consequence 严重性封顶）

**模板**:
> "The increase in the number of [phenomenon] in the past few decades is well documented in [source type]. According to [regulatory body], in [year], there was [quantified trend descriptor, e.g., 'an exponential increase in the number of...'] ([citation]). In [specific month year], [regulatory body 1], [regulatory body 2], and [firm] announced a [specific incident type] because of [specific defect] ([news citation]). The well-publicized [flagship case] of [firm] in [year] highlights that [phenomenon] can affect a significant proportion of [stakeholders] and that the defects connected with them can result in [severe consequence: loss of life and serious accidents]."

**来源**: kalaignanam_kushwaha_eilert2013 (JM), P1

**原文锚定**:
> "The increase in the number of product recalls in the past few decades is well documented in business reports. According to the Consumer Product Safety Commission (CPSC), in 2010, there was an exponential increase in the number of consumer products that were deemed hazardous and consequently withdrawn from the marketplace (CPSC 2010). In May 2007, the CPSC, the National Highway Traffic Safety Administration (NHTSA), and Evenflo Company Inc. announced a recall of Evenflo embrace infant car seat/carriers because of a malfunctioning handle (CNNMoney 2007). The well-publicized recall of Toyota in 2009 highlights that product recalls can affect a significant proportion of drivers and that the defects connected with them can result in loss of life and serious accidents."

**关键特征**:
- **严重性递进结构**：统计（抽象规模）→ 婴儿座椅（具体画面，但缺陷= malfunctioning handle，尚无伤亡）→ Toyota 2009（loss of life and serious accidents，严重性封顶）——三拍能量逐级升高，末拍落在最严重结果
- **多监管机构并置**："CPSC, NHTSA, and Evenflo" 三主体联合宣布——暗示问题的跨监管辖区严重性，比单一机构更有分量
- **新闻来源引用**：实例用 CNNMoney 而非学术引用——Hook 段允许新闻来源建立公共记忆，学术引用留给定义/趋势
- **与变体 I（chen2009）的分工**：变体 I 用 "ranging from A, to B, to C, to D" 的异质枚举展示**广度**（ubiquity）；本变体用"统计 → 单一实例 → 旗舰案例"的**严重性递进**。当现象有权威年度统计可用、且有一个公认的 high-stakes 旗舰案例时优先本变体
- **"well-publicized" 标记**：旗舰案例前必须加 public-visibility 修饰语，暗示"读者应该记得这件事"——触发公共记忆而非提供新信息

**适用**: 现象已有权威年度统计（CPSC/FDA/NHTSA 等）、存在一个公认的旗舰事件（Toyota 2009 / Vioxx / BP）、且需要把读者情绪从"知道很多"推向"这事很严重"的研究；Inadequacy × Mechanism 组合适配；JM/JMR 首选

**禁忌**:
- 旗舰案例必须是**公众已知**的——"well-publicized" 是前提，冷门案例无法触发公共记忆
- 三拍顺序不可调换——先统计后实例再旗舰，若先旗舰后统计则退化为新闻导语
- 实例与旗舰案例必须有**时间间隔**（Evenflo 2007 → Toyota 2009），暗示问题持续性；同年两例失去递进意义

---

### 变体 K：增长数据 + 行业双面性反转型（thirumalai2011 型）

**验证状态**: EMERGING（单篇来源；仅作 `section_variant`）

**功能节拍**: 行业增长证据（先进应用实例 → 监管法条定义 → 市场规模/增长率统计）→ "Although... sometimes they also become sources of significant risk" valence 反转 → 缺陷类型枚举 + 媒体佐证一句 → "on one hand... but on the other hand... bane" 双面收束

**模板**:
> "The ever-growing role of [industry product] in [domain] is evident from [concrete example of advanced application]. A [product] is '[regulatory statutory definition]' ([authority citation]). [Products], from [mundane example] to [extreme example], have become a necessary and critical component in [domain]. The [industry] is one of the fastest growing industries in the economy today. The overall revenue for the [industry] in [year] was estimated at about [$X], with an annual growth rate of about [Y]% ([citation]). Although [products] have become an indispensable component (often [lifesaving]) in [domain], sometimes they also become sources of significant risk to [users]. [Products], like other products, are prone to quality failures such as [defect type 1], [defect type 2], [defect type 3], and [defect type 4], which present a potential [risk] to [users]. Trade journals and newspapers abound with articles about [failure consequences]. Thus, on one hand, the [industry] has pushed the frontier of [positive outcome], but on the other hand, the quality problems with [products] have become the bane of this industry, resulting in [system-level loss]."

**来源**: thirumalai_sinha2011 (MS), P1-P2

**原文锚定**:
> "The ever-growing role of medical devices in the delivery of health care is evident from the wide array of medical equipment, gadgets, and complex machinery that are used across clinical settings ranging from small community care clinics to large super-specialty hospitals. A good example of this growth is the use of robotics in high-risk, high-precision surgeries today... The overall revenue for the medical device industry in the year 2006 was estimated at about US$90 billion, with an annual growth rate of about 6% (U.S. Department of Commerce 2008). Although medical devices have become an indispensable component (often lifesaving) in the delivery of health care, sometimes they also become sources of significant risk to the consumers of medical devices... Thus, on one hand, the medical device industry has pushed the frontier of superior health-care delivery, but on the other hand, the quality problems with medical devices have become the bane of this industry, resulting in ineffective health-care delivery."

**关键特征**:
- **增长面先行，风险面反转**：与变体 A-J 全部不同——数据建立的不是问题的规模，而是**行业的正面重要性**（lifesaving、$90B、6% 增长、robotics 尖端应用）；问题面由 "Although... sometimes they also become" 反转引入。张力来自同一行业的**双面共生**，而非问题本身的数量级
- **定义锚定用监管法条原文**：FDA 法条定义（"an instrument, apparatus, implement, machine..."）直接引用并附法条节号——比学术定义更具制度权威，且暗示"这是一个被监管定义的行业"，为后文监管主题（市场威慑失灵 + FDA 庇护）埋线
- **"from thermometers to artificial hearts" 谱系式枚举**：用 mundane→extreme 的产品谱系暗示行业覆盖广度与 stakes 跨度——与变体 I 的跨品类事故枚举不同，这里是同一行业内的产品谱系，无负面事件
- **"Trade journals and newspapers abound with..."**：媒体佐证一句带过、不引具体案例——风险面的证据强度刻意低于增长面（增长面用硬数据，风险面用媒体普遍性暗示），把量化 stakes 留给下一段
- **"on one hand... but on the other hand... bane" 对仗收束**：段末对仗把双面性定型为行业级张力——"bane"（祸根）是全文最强负面措辞，Hook 的情感高点落在段尾

**适用**: 研究行业本身具有内在双面性（lifesaving vs lethal、高增长 vs 高风险）；行业正面重要性有硬数据支撑（营收/增长率/就业），而问题面尚无系统量化证据（否则用变体 E/J 直接建立问题规模）；Inadequacy × Mechanism 组合；MS/MSOM/POM/JOM 等 OM 期刊适配

**禁忌**:
- 增长数据必须来自权威来源（U.S. Department of Commerce），不可用行业宣传材料——双面性的可信度依赖增长面的证据硬度
- 反转句中的 "**sometimes** they also become" 不可省略——它保持风险面措辞谦逊，避免把行业预设为系统性失败（本文最终发现市场不惩罚召回，行业不能被写成失败者）
- "bane" 级强措辞一篇 Introduction 只能用一次——后文 Stakes/Tension 段须降回分析性措辞
- 与 `15-extreme-situation` 的边界：制度不可能性（非法/污名行业）用 15；行业内部双面性用本变体——不要把"高增长+高风险"误路由到极端情境
- 风险面若已有权威量化证据（伤亡统计/事故率），应直接建立问题规模（变体 E/J），不要用双面性稀释冲击
...
> 最后更新: 2026-08-06 | 聚合论文数: 1

### 语气光谱
- **主语气**: matter-of-fact declarative — 证据: "Incidents of negative publicity are ubiquitous in the marketplace" (chen_ganesan_liu2009) — 断言式开场，不用问句不用惊叹

### 段落节奏
- **chen2009 节奏**: P1 枚举+命名+定义（~90 词）→ P2 统计+趋势升级（~80 词）→ P3 独立 Stakes 段（量化双案例）——Hook 自身保持轻量， stakes 外包给 P3

### 标志性叙事标记
- [chen_ganesan_liu2009]: "ranging from lead paint-contaminated toys, to faulty tires, to tainted pet food, to unhygienic food products" — 无数字画面枚举建立 ubiquity
- [chen_ganesan_liu2009]: "likely to occur more often in the future because of [4 drivers]" — 趋势升级把现象从现状提升为增长曲线

### 刻意回避
- [chen_ganesan_liu2009]: avoids opening with statistics — 统计数字后置到 P2，首段纯叙事
- [chen_ganesan_liu2009]: avoids theorizing in the Hook — 定义锚定用权威引用完成，不展开机制

### 质量标记
- **最值得模仿**: 枚举→命名→定义的三拍在 90 词内完成，构念界定零成本嵌入 Hook
- **已知风险**: 无独立 Stakes 时能量不足——必须后接量化案例段（chen2009 用 Merck/Topps），否则"so what?"悬空

### 模块比重参考
- Hook（含趋势升级）~15% / Stakes（量化案例）~10%（chen2009 Introduction 前 3 段）
- *来源: chen_ganesan_liu2009 (JM)*
