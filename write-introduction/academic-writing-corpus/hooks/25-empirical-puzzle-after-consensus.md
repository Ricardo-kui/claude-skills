...
先承认文献中已建立的共识关系，然后立刻用跨行业 anecdotal evidence 指出同一主体之间存在相反的、被忽视的结果。核心动作是"共识 → 反例 → 缺口声明"，能量中等，既展示对文献的尊重，又用具体公司名称建立真实感。

## 适用场景

- Gap 类型 = **Incompleteness**（文献关注了 A，但遗漏了与 A 并存的 B）
- 领域中已有关于 [X] 的明确正向/主流发现，但实践观察显示存在反向或并行的负面/竞争结果
- 有跨行业的具体案例可用于建立现象普遍性
- 目标期刊：SMJ、AMJ、JOM、JM
...
### 排他性
- **MEDIUM**: 主要用于 Incompleteness；与 Inadequacy 的 "blindspot" hook 有重叠，但本 Hook 强调经验悖论而非文献视角缺陷

---

## 句法模板

### 变体 A：共识+反例轶事型（cui_vertinsky 型）
...
**关键特征**:
- 首句建立文献共识（"has revealed that..."），用 3-4 个引用展示领域成熟度
- "However, ample anecdotal evidence indicates that..." 作为从共识到悖论的转折——不攻击文献，而是指出文献未涵盖的实践现实
- 使用具体公司名称和跨行业案例建立 Human Face 和现象普遍性
- 结尾 "Despite the abundant evidence, researchers have not paid adequate attention to..." 将悖论转化为可研究的学术缺口

**适用**: 当研究现象在实践中有丰富证据但在学术文献中处于边缘位置时；特别适合战略管理、组织间关系、竞争动态研究

**禁忌**: 不要在没有跨行业/多案例证据时使用单一 anecdote；反例必须与首句共识形成真实张力，不能是无关现象
...
### 必须配对
- **与 `01-despite-progress-unaddressed` (Tension) 配对**: 共识后的经验悖论需要递进式缺口解释为什么文献遗漏了该维度
- 或与 `16-threefold-gap` (Tension) 配对：当缺口可从多个独立维度展开时

### 互斥
- **不能与 `06-paradigm-challenge` (Hook) 同用**: 本 Hook 能量为中，与范式挑战的高能量冲突
- **不能与 `03-data-shock` (Hook) 同用**: 两者都试图建立"现实与文献不同"，但机制不同（anecdote vs 数据），会造成叙事焦点分裂

### 反模式提醒
- **不要只有反例没有共识**: 首句必须先建立文献共识，否则 Hook 退化为普通 anecdote
- **不要列举过多案例**: 2-3 个跨行业案例足够，超过 4 个会显得堆砌
- **"ample anecdotal evidence" 必须后续有具体案例支撑**: 不能先说"大量轶事证据"然后只给一个例子
...
| 期刊 | 适配度 | 注意事项 |
|------|--------|---------|
| SMJ | ⭐⭐⭐⭐⭐ | 战略管理研究首选；跨行业案例建立现象可信度 |
| AMJ | ⭐⭐⭐⭐☆ | 需强化理论重要性或实践后果 |
| JOM | ⭐⭐⭐⭐☆ | 运营管理中竞争/协作研究可用 |
| JM/JMR | ⭐⭐⭐☆☆ | 营销领域需补充市场结果 stakes |
| ASQ | ⭐⭐☆☆☆ | ASQ 偏好更强理论张力，本 Hook 偏温和 |
...
> 最后更新: 2026-07-08 | 聚合论文数: 1

### 语气光谱
- **主语气**: cautious-assertive — 证据: "has revealed that..." 后立即接 "However, ample anecdotal evidence indicates..."
...
- **已知风险**: "anecdotal evidence" 能量中等；若目标期刊要求更高能量，需补充量化数据或更强烈的理论冲突
