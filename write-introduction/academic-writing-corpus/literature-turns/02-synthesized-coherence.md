...
exclusivity: MEDIUM
source_papers:
  - gamache2020 (SMJ, 2020): "Two streams... developed largely in parallel"
...
# 02-synthesized-coherence — Synthesized Coherence 综合式文献对话

## 功能描述

P2-P3 的功能：连接多个文献流，展示它们共享一个盲区——构念的混淆、层次的误置、或边界的模糊。核心逻辑是"多个文献流各自抓到了现象的一部分，但都没有意识到它们共享一个盲点。"

这是 **Inadequacy** Gap 最常用的 Conversation 策略。

## 适用场景

- Gap 类型 = **Inadequacy**（现有视角抓住了现象，但误置了构念/层次/边界）
- 研究横跨两个或多个文献流
- 需要展示对话能力和整合视野
- 目标期刊接受综合式论证（ASQ, AMJ, SMJ, OS）
...
### 排他性
- **MEDIUM**: 主要在 Inadequacy 中出现，但也可用于 Incompleteness 的跨学科研究

---

## 句法模板

### 变体 A：多文献流连接型（gamache2020 型）
...
**关键特征**:
- "Two streams of research" → 明确多文献流结构
- "developed largely in parallel" → 指出未连接状态
- "leaving [gap] underexplored" → 精确指出交集盲区

---

### 变体 B：文献混淆识别型（pollock2015 型）
...
| AMJ | ⭐⭐⭐⭐⭐ | 需要清晰的机制链连接多个文献流 |
| SMJ | ⭐⭐⭐⭐⭐ | 偏好多层次贡献和多文献对话 |
| OS | ⭐⭐⭐⭐☆ | 需要强调 broader implications |
| JM/JMR | ⭐⭐☆☆☆ | 可用，但通常需要更快进入具体假设 |

---

## 相关语料

- 配合 `tensions/02-implicit-assumption-wrong.md` 使用：隐性假设错误是 Synthesized Coherence 的常见 Tension
- 配合 `contributions/_index.md` 中的 "构念混淆代价型" Stakes 使用
