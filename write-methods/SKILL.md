...
name: write-methods
description: >-
  顶刊 Methods 论证结构生成器——输入设计类型输出带 [placeholder] 的可直接粘贴段落（M1-M10 槽位；覆盖 23+ 设计类型）。Use when writing the methods section of a management-journal paper。Not for: 蒸馏范文（→ distill-methods-exemplar）；审查草稿（→ methods-review）；稳健性结果正文（→ write-results）。分工：识别策略的理论论证属 write-theory，实现属本 skill；实验/多研究设计属本 skill，结果与跨研究综合属 write-results。
---
...
1. **paper-state.yaml 自动消费（推荐）**：按 `--paper-state=<path>` → 当前目录 → 项目根目录查找；检测到后验证 canonical `story`，读取 `theory.constructs` 和 `theory.hypotheses`，自动生成 storyline–hypothesis–variable mapping（用户只需确认变量操作化方式）。`theory.hypotheses` 为空 → 回退方式二。
2. **write-theory 输出文本消费（回退）**：`假设列表` → 假设-变量映射表；`核心构念` → 变量操作化模板。
...
**批评登记**：用户不满时登记到 `econometric-models/_evidence_registry.yaml`（`evidence.by_design_type.<类型>.validation_history`：revise/reject +1、last_critique=今天、reasons 去重首插最多 8 条）；只登记变体产出质量批评，不登记 [placeholder] 流程抱怨与风格偏好；批评只落 registry 不自动改 corpus（由 distill-methods-exemplar 选材 Gate 驱动精炼）。批量补登可用 `python _update_registry.py --record-critique <critiques.yaml>`。

**语料与变体**：设计类型具体变体见 `econometric-models/[设计类型].md`；新论文蒸馏结果经 `distill-methods-exemplar` → Phase 4 自动写入（同步更新 INDEX.md 变体数）。

---
*基于 34 篇 MVP30 范文语料库、Pollock 2025 Ch07 构建。v3.5.0（2026-08-09 信息层级重构：SKILL.md 452→150 行，8 个 references 下沉）。*
