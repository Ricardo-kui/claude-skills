...
### 变体 1: 同时方程变体
： 🔬 EXPERIMENTAL（1-2 篇范文）⚠️ 保守替代：通用 M7 段落
...
### 变体 1: 同时方程/方程特定控制变体
： 🔬 EXPERIMENTAL（1-2 篇范文）⚠️ 保守替代：通用 M6 段落
...
### 变体 2: 控制变量表格呈现型（多方程映射 + Table Notes）
...
```text
Because our model involves [endogeneity concern: simultaneity / omitted variables / reverse causality], we must choose between [simple estimator] and [complex estimator]. We follow the established procedure of estimating both and selecting the preferred estimator based on [diagnostic test]. First, we estimate the system using [simple estimator]. Second, we estimate the system using [complex estimator] with [instrument specification / additional assumptions]. We then apply the [Durbin-Wu-Hausman / Hausman / Sargan-Hansen] test to compare the two estimators. The [nonsignificant / significant] test statistic (χ² = [value], p [relation] [threshold]) indicates that [simple estimator] is [preferred / rejected] and [complex estimator] does [not] provide efficiency gains [or is / is not necessary to address endogeneity]. We report the [simple / complex] estimator as our preferred specification. As a robustness check, we also report results using the [alternative estimator] in [location].
```

**设计变体**：
- **SUR vs 3SLS 选择模式**（如 JM 2015 Malshe）：先估计 SUR，再用 3SLS 加 IV；DWH test 判断是否拒绝 SUR。若 DWH 不显著，选 SUR（更简洁、更高效）。
- **FE vs RE 选择模式**（已有主骨架）：Hausman test 选择 FE，适用于面板数据。
- **OLS vs IV 选择模式**：DWH test 判断是否拒绝 OLS。若拒绝，说明内生性存在，需用 IV。
