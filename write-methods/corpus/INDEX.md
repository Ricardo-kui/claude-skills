...
| [动态面板-GMM](动态面板-GMM.md) | 动态面板-GMM | 5 | 2026-08-13；| 6 | 短面板适用性 + 估计器优势枚举 | 审稿人未点名内生性威胁时的正面轻量辩护；短面板 ADL 场景 | 区别变体 1（威胁先行统一解）：适用性先行 + First/Second/Finally 优势列举（Nickell 型 LDV、稳健 SE、深度滞后工具） | EMERGING | Ridge, Aime & White 2013 (SMJ) |；| 7 | AB-GMM 诊断对报告链（AR(2)+Hansen） | 估计后诊断报告段 | 区别变体 3（事前工具精调）：本变体是事后三拍报告——零假设白话陈述→违例含义（哪些工具失效）→判定收口 | EMERGING | Ridge, Aime & White 2013 (SMJ) | |
| [匹配DiD-广义DiD](匹配DiD-广义DiD.md) | 匹配DiD-广义DiD | 1 | 2026-08-05；变体 2. M2：对照组从同一对冲击机构的其他持仓中抽取（同机构持有+无处理变化双条件），吸收机构选股风格差异；区别于 CEM 匹配变体（那是事后协变量匹配，这是冲击内部结构化对照）。 |
