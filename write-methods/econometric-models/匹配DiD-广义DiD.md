...
| # | 变体 | 适用场景 | 区别 | 状态 | 来源 |
|---|---|---|---|---|---|
| 1 | CEM 匹配 ex-ante 价值代理 + 风险代理（准实验 DiD 稳健性） | 政策/交错 DiD 主识别之后，需用匹配抗辩 ex-ante 可比性、威胁声明与匹配维度一一对应（价值代理、风险代理）时（槽位 M8） | 区别于 `PSM匹配面板` 的 CEM 五步链——说服焦点是威胁声明→双代理匹配维度→CEM→稳健性位置，而非匹配算法细节 | 待交叉 | Castellaneta, Conti & Kacperczyk 2017 SMJ |
