...
8. **因果语言档位以匹配范文为基准**：动词档位按匹配设计家族范文实际用法定档（实验可用强因果词；自然实验默认识别支持后 "effect of... on..."；观察性设计默认审慎动词）；`corpus/micro-templates/causal-hedging.md` 仅作默认参考。商科期刊对观察性设计的因果动词没有统一强制标准，不得按软件工程/计算机标准机械执行 identified→estimated 类审查（feedback-registry wmf_206116a4）。
