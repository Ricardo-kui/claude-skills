<!-- write-methods 槽位骨架 M1：由 SKILL.md「槽位骨架加载」按路由决策加载。内容为原 SKILL.md 该槽位段落骨架（通用 + 设计类型变体 + QC），未做语义修改。状态词表单源：corpus/INDEX.md（ROBUST > VERIFIED > EMERGING；LEGACY-DIAGNOSTIC 保留）。 -->

### M1. 研究情境 / 实证背景

**通用填空段落**： ⭐ ROBUST（28/28 篇范文使用，跨所有模型类型复现）
...
**自然实验/DiD 变体**（替换首句）： ✓ VERIFIED（5-8 篇 DiD/自然实验范文复现）
```text
...
**Staggered DiD + 二元结果 变体**（hoffmann_cheong_phan_zurbruegg2024 型，替换首句）： 🔬 EMERGING（1 篇范文，2026-08-05 重蒸馏）⚠️ 保守替代：自然实验/DiD 变体 + M7 非线性模型变体
```text
...
**多研究变体**： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：省略
```text
...
**同时方程/SEM 变体**（替换整个 M1）： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：通用填空段落 + M7 同时方程变体
```text
...
**质性→量化混合方法变体**（Haunschild et al. 2015 ORSC 模式）： 🔬 EMERGING（1 篇范文）⚠️ 保守替代：通用 M1 段落
```text
