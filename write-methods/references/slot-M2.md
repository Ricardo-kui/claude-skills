<!-- write-methods 槽位骨架 M2：由 SKILL.md「槽位骨架加载」按路由决策加载。内容为原 SKILL.md 该槽位段落骨架（通用 + 设计类型变体 + QC），未做语义修改。状态词表单源：corpus/INDEX.md（ROBUST > VERIFIED > EMERGING；LEGACY-DIAGNOSTIC 保留）。 -->
...
**稀有结果变体**（在通用段落前插入）： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：通用 M2 段落 + 脚注说明抽样策略
```text
...
**实证对象构建变体**（替换或前置）： 🔬 EMERGING（2-3 篇范文）⚠️ 保守替代：通用 M2 段落
```text
...
**自然实验/DiD 变体**： ✓ VERIFIED（5-8 篇 DiD 范文复现）
```text
...
**Staggered DiD 样本周期双重辩护变体**（hoffmann_cheong_phan_zurbruegg2024 型）： 🔬 EMERGING（1 篇范文，2026-08-05 重蒸馏）⚠️ 保守替代：自然实验/DiD 变体
```text
...
**多研究变体**（逐研究）： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：通用 M2 段落 + M9 多研究过渡段
```text
...
**PSM匹配面板变体**（在通用段落中加入匹配步骤）： 🔬 EMERGING（2-3 篇范文）⚠️ 保守替代：通用 M2 + M8 匹配检验
```text
...
**层级回退匹配变体**（如 Pfarrer et al. AMJ，1:3 SIC 匹配 + 层级回退）： 🔬 EMERGING（2 篇范文：Pfarrer et al., Mayo et al.）⚠️ 保守替代：通用 M2 + PSM 变体
```text
...
**多行为者设计变体**（替换通用段落）： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：通用 M2 段落 + 说明多数据源匹配
```text
...
**多源嵌套调查变体**（如 Mannor et al. SMJ，多方法数据 + 聚类标准误）： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：通用 M2 + M7 多层模型/聚类标准误
```text
