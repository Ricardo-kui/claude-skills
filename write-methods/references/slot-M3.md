<!-- write-methods 槽位骨架 M3：由 SKILL.md「槽位骨架加载」按路由决策加载。内容为原 SKILL.md 该槽位段落骨架（通用 + 设计类型变体 + QC），未做语义修改。状态词表单源：corpus/INDEX.md（ROBUST > VERIFIED > EMERGING；LEGACY-DIAGNOSTIC 保留）。 -->
...
**稀有结果/序数变体**（替换末句）： 🔬 EMERGING（2-3 篇范文）⚠️ 保守替代：通用 M3 段落
```text
...
**指数/净指数变体**： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：通用 M3 段落
```text
...
**行为编码变体（实验）**： 🔬 EMERGING（3-4 篇实验范文）⚠️ 保守替代：通用 M3 段落 + 说明编码者间信度
```text
...
**文本构念测量变体**（M3 或 M4 均可使用，三段式效度链）： 🔬 EMERGING（3-4 篇范文：Zhao 2022, Gamache 2020 等）⚠️ 保守替代：通用 M3 + 增加效度检验句
```text
...
**LIWC 心理语言学构念测量变体**（如 Mannor et al. SMJ，Pfarrer et al. AMJ）： 🔬 EMERGING（2-3 篇范文）⚠️ 保守替代：通用 M3 段落 + 增加字典说明
```text
...
**人工内容分析 + 编码者间信度变体**（如 Desai AMJ，Pfarrer et al. AMJ）： 🔬 EMERGING（2-3 篇范文）⚠️ 保守替代：通用 M3 段落 + 编码者间信度说明
```text
...
**测量防御三段式变体**（Pontikes 2012 模式：承认局限 → 论证最优可用 → 保守检验逻辑）： ✓ VERIFIED
...
**替代测量效度三角变体**（Haunschild et al. 2015 ORSC 模式）： 🔬 EMERGING（1 篇范文）⚠️ 保守替代：通用 M3 段落
```text
...
**非 WEIRD 语境 DV 调校变体**（Li et al. 2026 POM 型）： 🔬 EMERGING（1 篇范文）— 当标准（WEIRD 语境）DV 的制度前提在非 WEIRD 语境不成立时，换用语境适配 DV 并用本地证据校准窗口/阈值
```text
...
**产品伤害危机的监管指标 + 召回边界声明**（Kashmiri, Nicol & Arora 2017 JAMS 型）：🔬 EMERGING（单篇来源）
