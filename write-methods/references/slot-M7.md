<!-- write-methods 槽位骨架 M7：由 SKILL.md「槽位骨架加载」按路由决策加载。内容为原 SKILL.md 该槽位段落骨架（通用 + 设计类型变体 + QC），未做语义修改。状态词表单源：corpus/INDEX.md（ROBUST > VERIFIED > EMERGING；LEGACY-DIAGNOSTIC 保留）。 -->
...
**模型选择理由补充段**（按需添加）： ✓ VERIFIED（15+/28 篇范文使用）
```text
...
**非线性模型变体**： ✓ VERIFIED（8-10 篇非线性模型范文复现）
```text
...
**计数模型负二项变体**（Haunschild et al. 2015 ORSC 模式）： 🔬 EMERGING（1 篇范文）⚠️ 保守替代：非线性模型变体
```text
...
**Staggered DiD + 条件 Logit 变体**（hoffmann_cheong_phan_zurbruegg2024 型，二元结果 + 交错处理时点 + year/industry FE）： 🔬 EMERGING（1 篇范文，2026-08-05 重蒸馏）⚠️ 保守替代：DiD 变体 + 非线性模型变体
```text
...
**生存分析变体**： 🔬 EMERGING（2-3 篇范文：Zhou 2017, Pontikes 2012 等）⚠️ 保守替代：通用 M7 段落 + 说明分布选择
```text
...
**复发事件 AFT 变体**（当同一主体经历多次事件时）： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：通用 M7 + 生存分析变体
```text
...
**IV/2SLS 变体**： ✓ VERIFIED（3-4 篇 IV 范文复现）
```text
...
**策略性内生性变体**（当核心解释变量是行动者主动选择时）： ✓ VERIFIED（IV/控制函数/自然实验范文通用）
```text
...
**IV/2SLS 多结果对称变体**（同 IV，多个相关 second-stage 结果）： ✓ VERIFIED（双结果/利益相关者反应研究常见）
```text
...
**线性概率模型（LPM）+ 2SLS 变体**（二元 DV 且需固定效应时）： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：IV/2SLS 变体
```text
...
**动态面板/GMM 变体**： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：通用 M7 段落 + M8 Nickell bias 提示
```text
...
**匹配DiD/广义DiD 变体**： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：DiD 变体 + M2 PSM 变体
```text
...
**堆叠扩散Logit 变体**： 🔬 EMERGING（1 篇范文）⚠️ 保守替代：通用 M7 段落
```text
...
**PSM匹配面板 + 随机效应Tobit 变体**： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：M7 Tobit + M2 PSM
```text
...
**HLM/多层模型变体**（当数据为嵌套结构，如员工-团队-公司，或重复测量-个体时）： 🔬 EMERGING（1-2 篇范文）⚠️ 保守替代：通用 M7 段落 + 说明聚类标准误
```text
