...
**类型内变体选择**：打开 `econometric-models/[结果类型].md`，**先读顶部「变体速查表」**——按槽位+验证状态（通过（N/5 复现）> 通过（双篇/专家审计）> 通过（单篇）> 待第二篇交叉验证 > 可选变体）定位候选，再精读对应变体正文（骨架/诚实边界/跨 skill 对齐）。速查表与正文变体由 quality_check 校验同步。
