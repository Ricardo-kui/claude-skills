...
**语料优先改编（段落确定后）**：每个段落一旦确定证据功能，以对应 slot 骨架 + 结果类型变体的语料句式为改编底本——尽量使用语料库的句式表达来改编（替换来源特异性内容），而非自拟；无对应变体时按骨架生成。已核实事实与用户裁定优先于语料句式（见路由 Step 3）。**跨节句子功能查询**（想借他节功能句/词汇——hook 句、because 从句、band 词表等）→ 读 `../_shared/function-map.md`（功能→载体地图）。
