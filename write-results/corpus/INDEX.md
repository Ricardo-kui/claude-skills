...
| 文件 | 结果类型 | 变体数 | 最后更新 |
|------|---------|--------|---------|
| [OLS-FE](OLS-FE.md) | OLS-FE | 99 | 2026-09-12；变体 63：R3/R5 ln(时长) DV 跨列选择性显著 + 天数回译幅度拍，wowak_2020_female_directors_recalls，VERIFIED (expert_audit_override 2026-08-28 产品召回主研究领域单源足矣)；变体 64：R3 双处理对照四拍 + Wald 系数差检验 + 跨 DV 合并幅度，post_2022_women_tmt_strategic_renewal，通过（专家审计单源；变体 65：R4 分样本镜像对照 + 组内 Wald + Chow 诚实降级，post_2022_women_tmt_strategic_renewal，通过（专家审计单源）；变体 66：R7 三威胁小节化稳健性（选择性→替代估计→构念效度），post_2022_women_tmt_strategic_renewal，通过（专家审计单源）；变体 67：R1 双路径前提描述统计（理论预言零相关 + moderator 分布/条件定义），post_2022_women_tmt_strategic_renewal，通过（专家审计单源）；变体 68：R4 交互通道分解句（调节经差值 DV 哪个分量起作用+双分量佐证），westphal_bednar2005，VERIFIED；变体 69：R8 理论前提实证验证双通道（样本内切割检验+Heckman 交底；样本外前提问卷+K-S），westphal_bednar2005，VERIFIED；变体 70：R2 先验支持判据声明（判定规则先于结果交代+检验变量编码），westphal_bednar2005，VERIFIED）；2026-08-29 fang2025 POM 新增 2 变体：R3 显式算术除法幅度拍（β×unit=amount → amount÷base=%）、R6 null 作为竞争策略裁决证据（supports A, while supporting neither B nor C 三策略一次收束；变体 71：R2 估计策略+分样本设计导航段，carpenterwestphal2001，VERIFIED（单篇）；变体 72：R3 多指标假设裁决四拍（3 显著 + 1 方向分），carpenterwestphal2001，VERIFIED（单篇）；变体 73：R4 分样本主检验+交互项复核句（方向反转），carpenterwestphal2001，VERIFIED（单篇）；变体 74：R6 跨情境镜像 null 收束句（opposite pattern），carpenterwestphal2001，VERIFIED（单篇）；变体 75：R8 连续 DV legacy Baron–Kenny 中介三步句（LEGACY），carpenterwestphal2001，VERIFIED（单篇）；变体 76：Heckman 选择模型前置交底（higgins_2003_OS；威胁类比定位+两阶段程序+风险集/SE 修正交底前置到主结果之前；vs 24/28 表导航、29 三步防御、54 RIR/2SRI 前置——本变体是主估计器动机论证而非稳健性防御；VERIFIED）；变体 77：假设判决链（higgins_2003_OS；假设重述嵌入拍1+模型定位拍+判决拍，null 判决用 However 衔接相邻假设判决成段；vs 变体36/58 管统计判定不管判决段落编排；幅度拍缺位警示——采用时按 R3 主骨架补齐；VERIFIED）；变体 78：Baron-Kenny+Sobel 假设化部分中介跨 DV 重演（higgins_2003_OS；(a)(b)(c) 三条件逐一列举+Sobel 乘积系数 p 值+跨多 DV 表重演+partial-vs-full 系数下降幅度裁决；vs 58 现代 bootstrap 堆叠——本变体是其 legacy 前身须标 legacy 并补间接效应区间、41 SUR 非对称、56 legacy 完全中介；VERIFIED）；变体 79：理论动机补充分析双段式（higgins_2003_OS；'since our claims centered on X' 理论开题+边界条件交互+构念加总层次裁决，明标 Additional Analyses 与 threat 稳健性分离；vs 21 枚举清单、12 跨样本复制、33 下游绩效展演；VERIFIED）；变体 80：跨分样本/跨模型系数比较基础设施声明（比较逻辑预告→分别估计→SUR 联合协方差→括号双轨显著标记→列子集导航；H3-H5 型；gulati2005-adaptation-vertical，VERIFIED，switching-regression 族暂落 OLS-FE 族）；变体 81：不支持裁决 + 以假设语言重述实际模式的双拍诚实收束（假设回指→各组证据含边沿/相反→明确 not supported 裁决→模式重述；H4 型 null 处理；gulati2005-adaptation-vertical，VERIFIED）；变体 82：假设判决驱动的系统内 BK 三步中介+拟合改善拍+Sobel+联合模型 nearly full mediation 校准（gulati_sytch2007，VERIFIED；为变体75/78 legacy BK 族第二源，强制标 LEGACY 并补 bootstrap CI 边界）；变体 83：配对反号假设反例合并判决——反号显著+配对 null 一句并置+Hence 合并不支持+显式推迟 Discussion（gulati_sytch2007，VERIFIED；为变体81 null 判决族第二源）；变体 84：R5 四分位差幅度翻译（25th→75th → 占 SD 百分比）+ R7 跨规格幅度重算与排位（lies between …）——幅度缩水如实报并给解释句（desjardine2025 型）；变体 85：R7 Heckman 选择偏倚交底——DV 覆盖选择性来源 + 两条 exclusion restrictions 命名报告 + inverse Mills 不显著收口（desjardine2025 型）；变体 86：R4 连续调节极点→交互符号先验判据（gulati_higgins2003smj 型）——判据拍内嵌假设段；区别于变体70（R2 前置 dummy 差值判据）与变体77（无判据拍判决链）；变体 87：R3 判决先行假设段（gulati_higgins2003smj 型）——判决作段落主题句；区别于变体77（姊妹篇 Higgins & Gulati OS 2003 的重述先行判决链）；变体 88：R2 全交互饱和模型存活复核句（gulati_higgins2003smj 型）——逐假设模型后的统一复核拍；区别于变体53（逐调节→成对→全模型 8 模型全程导航）；变体 89：R3+R9 主效应×交互 tandem 收束句（gulati_higgins2003smj 型）——标题问句回环双维收束，主效应降位并置；区别于变体27（多阶段管道跨阶段对比收束）；变体 90：对照假设成对判决节奏（rival null 先行 + A rather than B 收束），Gulati_Lavie_Singh_2009_SMJ，EMERGING；变体 91：相对排序假设 within-model 系数比较 F 检验（one-sided F + 不等式判决），Gulati_Lavie_Singh_2009_SMJ，EMERGING；变体 92：主效应被自身交互项共线吸收解释 + df 归因（与变体48 交互侧互补），Gulati_Lavie_Singh_2009_SMJ，EMERGING；变体 93：双侧对照交互判决 + 对手侧 null 回收为确证（consistent with our conjecture），Gulati_Lavie_Singh_2009_SMJ，EMERGING；变体 94：控制块 null 先例锚定开场（mostly insignificant + 文献同向 null），Gulati_Lavie_Singh_2009_SMJ，EMERGING；变体 95：意外反号主效应原位 hedge 重读（may suggest 一句话，解释留给 Discussion），Gulati_Lavie_Singh_2009_SMJ，EMERGING；变体 96：低解释力文献基准防御 + 分析目的重定位（R² 区间→文献更低基准→contribution 而非 variance），Gulati_Lavie_Singh_2009_SMJ，EMERGING；变体 97：R3 U 形主效应四拍 + 双尾 ±2 SD 实用显著性（上尾/下尾各一次 % 幅度 + 平均公司绝对美元换算）+ 双机制接管重述收口，Ridge, Aime & White 2013 SMJ，EMERGING——区别于计数模型倒U链（转折点 CI 现代裁决）与本文件 IQR 经济显著性变体：本变体是 U 形双尾各自换算美元的实用显著性翻译；变体 98：R4 曲线调节机制判别式（X²×W 检验主效应右臂机制），Ridge, Aime & White 2013 SMJ，EMERGING——区别于本文件线性调节变体（±1 SD simple slopes）与 Logit 概率尺度曲线调节：诊断目的重述句 + 无/有 W 对照幅度 + 图形读形状（does not turn positive）+ 回锚主假设对应臂；变体 99：R5 双机制分臂量化（调节强化 U 形两臂时逐臂报幅度：机制 A 臂 + 机制 B 臂，多调节均值+括号分项），Ridge, Aime & White 2013 SMJ，EMERGING——区别于 R4 线性调节实用项与 R5 百分位对比变体：两臂各算一次账且均值+分项并报，防只报最好调节） |
| [Logit-Probit-Ordered-Probit](Logit-Probit-Ordered-Probit.md) | Logit-Probit-Ordered-Probit | 43 | 2026-08-13；变体 O：首事件建模范围辩护（理论假设+反转占比经验双轨 + In effect 理论任务收束句），westphal_zajac_1998_symbolic_management，VERIFIED (expert_audit_override 2026-08-28)；变体 P：四格 null 格确认性报告（In contrast 正面报告 null + generally 汇总措辞 + 双 DV 并轨导航），westphal_zajac_1998_symbolic_management，VERIFIED (expert_audit_override 2026-08-28)；变体 Q：四格系数 Wald 正式确认（to confirm 升级排序证据 + 双 DV 双统计量并列 + e.g. 代表例），westphal_zajac_1998_symbolic_management，VERIFIED (expert_audit_override 2026-08-28)；变体 R：双 DV 收敛现造理论标签（Taken together + not only/but also 递进 + whereby 边界从句），westphal_zajac_1998_symbolic_management，VERIFIED (expert_audit_override 2026-08-28)；变体 S：滞后结构括弧敏感性（短/长双向夹逼 + 前向结果定位 + 不敏感性结论复述），westphal_zajac_1998_symbolic_management，VERIFIED (expert_audit_override 2026-08-28)；变体 T：普遍信念反转收束段（despite widespread belief 让步对撞 + suggest 克制强度 + 机制级命题聚合），westphal_zajac_1998_symbolic_management，VERIFIED (expert_audit_override 2026-08-28)；变体 U：R4 非参数等价裁决+基线符号对比段节奏（pfarrer2010 型，R4）——处理组 vs 基线的分组比较裁决段四拍：whereas 符号对比（处理组微正 vs 基线显著负）→ 非参数检验把『处理组结果≈预测值』与『基线显著偏离』分开陈述 → t 检验组间差异 → 拆分裁决（Thus, H a and H b supported, H c not）；与 r3_eventstudy_nonparametric_validation（主比较前的 sanity check）和 r4_subgroup_mean_comparison_ttest（H3 表的正面对比）分工：本变体是裁决段内部的第二检验家族三角；变体 V：R4 差值列 worked-arithmetic 脚注（pfarrer2010 型，R4）——分组比较表脚注用一个数值示例（2.30%−1.74%=0.56%）教读者差值列的算法，同脚注打包显著性检验类型（t-test of unequal variances）与基线类目操作化定义（none = 双维度均 0）；与 r4_subgroup_mean_comparison_ttest（正文裁决节奏）分工：本变体是表格脚注层的可读性装置；变体 W：R2 DV 家族换场小节标题（pfarrer2010 型，R2）——Results 在两个 DV 家族（事件发生概率 logit → 投资者反应 CAR）之间换场时，用不带编号、构念命名的子小节标题（Effects of A and B on ...）开启新证据流，首句整组重述新假设集（Hypotheses Xa–Xc and Ya–Yc predict the effects of...）；与 r2_binary_outcome_hypothesis_pair_table（双表镜像导航）分工：本变体是跨 DV 家族的结构性换场装置；变体 X：R2 相关替代测量分模型括弧（高相关→分模型 two alternative tests+2×2 网格+似然拟合增量拍），gulati_1999_network_location_and_learning_the_influence_of_n，VERIFIED (expert_audit_override 2026-09-05)；变体 Y：R2 控制变量解释消去（基线显著→焦点变量入场衰减→was capturing 正向重读+功能形式收束），gulati_1999_network_location_and_learning_the_influence_of_n，VERIFIED (expert_audit_override 2026-09-05)；变体 Z：R6 构念家族混合证据（mixed 标题拍+核心操作化四拍+辅助操作化逐项 null 全披露），gulati_1999_network_location_and_learning_the_influence_of_n，VERIFIED (expert_audit_override 2026-09-05)；变体 AA：R7 随机效应方差份额自证（Rho 定义拍→跨模型显著→纠正生效+FE 一致性搭配），gulati_1999_network_location_and_learning_the_influence_of_n，VERIFIED (expert_audit_override 2026-09-05)；变体 AB：R7 截距差异≠斜率异质性（概念区分拍→无约束分组模型→主效应方向同质性+第二分区重复），gulati_1999_network_location_and_learning_the_influence_of_n，VERIFIED (expert_audit_override 2026-09-05)；变体 AC：R2 假设级风险集递进导航——dyad 设计每假设各配可估样本（gulati_westphal_1999 型；变体 AD：R6 null 主假设前置 pivot——判决先行冒号句 + In summary 权变化收束（gulati_westphal_1999 型）；变体 AE：R4 对称放大调节镜像判决句——单 moderator 同向放大异号双主效应（gulati_westphal_1999 型）；变体 AF：R3 双估计器并列共证——logit+Heckman 双表导航 + 逐假设一行确认（gulati_westphal_1999 型）；变体 AG：R8 探索性中介五拍弧——供认→竞争机制电池→系数衰减判定→反向因果反驳（gulati_westphal_1999 型）；| AH | R6 竞争机制衰减裁决（基线设伏+拟合增量+非对立供认+趋势重读） | rival 理论关键变量在基线模型设伏、在焦点模型处被焦点构念吸收时，升级为两套实质理论的整体裁决 | 区别于变体 Y（控制衰减正向重读）——本变体是 rival 理论整体裁决且含事后性供认拍 | VERIFIED | Gulati 1999 AJS |；| AI | R6 null 主效应条件重估与构念域重划（less conclusive 降级+分离重估+子域收束） | null 主效应在原位降级判决后作分离重估并把机制重划为只适用于子群，null 不翻案只划界 | 区别于变体 AD（null 首位 pivot）——本变体是原位降级+重估+重划三步法 | VERIFIED | Gulati 1999 AJS |；| AJ | R4 预期符号翻译+共线性分模型声明+主效应存活综合 | 多交互项共线时先翻译预期符号再声明分模型检验，判决后以主效应存活句收束成双驱动综合 | 区别于变体 AE（镜像判决句）——本变体靠符号翻译+存活句承载权变主张，无图无 AME | VERIFIED | Gulati 1999 AJS |；| AK | R6 null 分量+显著交互坦白段（让步从句+弱证据边界+双向 hedge） | 已获支持的调节假设内部出现 null 分量时：坦白 null、解读交互、标注观察期证据边界 | 区别于变体 AD（null 首位 pivot）——本变体是组件级坦白而非翻案 | VERIFIED | Gulati 1999 AJS |；| AL | R7 曲线形式脚注稳健性（差异化发现+线性不受影响+简约保留裁决） | 理论以线性为主时把多项式敏感性检查放脚注：逐预测子报差异化曲线发现+不变性拍+简约保留裁决 | 区别于变体 9/11（曲线作主分析/六威胁梯）——本变体是脚注紧凑型 | VERIFIED | Gulati 1999 AJS |；| AM | R3 判决后构念效度同调拍（congruent 同调句+替代测量一行复证） | 主假设判决后以文献同调升格构念效度主张，并用替代测量一行复证收尾 | 区别于变体 X（替代测量分模型架构）——本变体是一行复证 coda | VERIFIED | Gulati 1999 AJS |）；变体 AN：内生转换回归第一阶段选择模型的中间步骤导航（两步骤预告→IV 排他声明→基线类解释→IV 相关性与样本损耗→自有价值旁注；gulati2005-adaptation-vertical，VERIFIED，switching-regression 族暂落 Logit 族；变体 AO：R2 形式模型决策树→二元回归元构造（支路 case dummy + 余集归池），mao_dong_lee_2022_msom，gap HIGH；变体 AP：R3 理论支路组间对比判读（显著/不显著组分读 + 合并判决 + 决策树回锚），mao_dong_lee_2022_msom，gap HIGH；变体 AQ：R7 阈值定义列敏感性（定义窗宽三档并排主表 + largely consistent 判决），mao_dong_lee_2022_msom，gap HIGH；变体 AR：R2 二元 DV 机构行为代理辩护（制度机制链 + 占比交底），mao_dong_lee_2022_msom，gap HIGH；变体 AS：R3 构念子节系数→假设映射判决（probit 无幅度版：likelihood 方向拍+判决措辞轮换+构念级理论收尾拍；幅度外包图形轨），Liu_Liu_Luo_2016，EMERGING；变体 AT：R6 多部件假设合并判决（部件异质显著性档并报不降档：一假设号跨两交互系数，marginal 部件原样显示并入支持；变体 25 spillover-null 的支持侧镜像），Liu_Liu_Luo_2016，EMERGING） |
| [生存分析](生存分析.md) | 生存分析 | 19 | 2026-08-01 |
| [DiD](DiD.md) | DiD | 19 | 2026-09-05 castellaneta_conti_kacperczyk2017 重蒸馏新增 2 变体：变体 O（R1 首填）DiD 描述统计叙述——DV 分布 + 处理组占比/暴露窗一句三事 + 地理行业分布，EMERGING；变体 P（R7）调节变量测量内生性四步防御（层次分离→pre-shock 构造外生→时间不变测量→rival 交互控制），EMERGING；此前 2026-08-29 fang2025 POM 新增 2 变体：R2 RDiT 事件前后 model-free 时间序列图开篇（成分分解 + 显著性入图解读 + 附录 t 值 + 过渡到估计）、R8 跨事件复制泛化（第二事件同方程重估 + 双偶然性排除收束 limited to neither A nor B） |
| [计数模型](计数模型.md) | 计数模型 | 21 | 2026-08-12；变体 22：计数模型主效应四拍 + FE/RE 双模型平行复现，ball_2018，gap HIGH；变体 23：DV-split 调节 + 跨列系数对比（无交互项），ball_2018，gap HIGH；变体 24：exp(β) 乘法解释批量幅度翻译独立段（正负系数各演一遍），ball_2018，gap HIGH；变体 25：四 threat 前瞻式总起 + 逐 threat 收束句（中介甄别/PSM/安慰剂DV/反向因果），ball_2018，gap HIGH；变体 26：R3 聚合列显著→严重度分列定位承载类，wowak_2020_female_directors_recalls，VERIFIED (expert_audit_override 2026-08-28 产品召回主研究领域单源足矣)；变体 27：R8 剂量—反应阈值发现（Wald χ² → tipping point），wowak_2020_female_directors_recalls，VERIFIED (expert_audit_override 2026-08-28 产品召回主研究领域单源足矣)；变体 28：R1 假设键控描述统计叙述（计数模型），anand_mukherjee_2024，VERIFIED；变体 29：R2 三步递进架构+检验驱动估计器正当化链（计数模型），anand_mukherjee_2024，VERIFIED；变体 30：R6 主效应仅全模型显著→保守判不支持（计数模型），anand_mukherjee_2024，VERIFIED；变体 31：R7 计数双威胁 IV 排他性散文+GEE exchangeable（计数模型），anand_mukherjee_2024，VERIFIED；变体 32：R8 替代注意事件机制三角+跨DV交叉学习折扣竞争解释，anand_mukherjee_2024，VERIFIED；变体 33：R8 post hoc 记忆衰退时窗+编码化解释（计数模型），anand_mukherjee_2024，VERIFIED；变体 34：R9 假设-构念映射汇总表 headline answer（计数模型），anand_mukherjee_2024，VERIFIED；变体 35：R7 CEM 匹配样本确认子节 — 全假设电池匹配复制 + 匹配数/粒度权衡，fini2017，gap HIGH |
| [实验](实验.md) | 实验 | 5 | 2026-08-03 |
| [多研究](多研究.md) | 多研究 | 8 | 2026-08-02 |
| [定性过程研究](定性过程研究.md) | 定性过程研究 | 4 | 2026-07-07 |
| [IV-2SLS](IV-2SLS.md) | IV-2SLS | 10 | 2026-08-05；变体 11：R7 排他性约束量化暴露占比辩护，wowak_2020_female_directors_recalls，VERIFIED (expert_audit_override 2026-08-28 产品召回主研究领域单源足矣)；变体 12：R7 弱识别临界值协议报告（Cragg-Donald vs Stock-Yogo），wowak_2020_female_directors_recalls，VERIFIED (expert_audit_override 2026-08-28 产品召回主研究领域单源足矣)；变体 13：R7 非线性主模型下线性 2SLS 稳健性轨，wowak_2020_female_directors_recalls，VERIFIED (expert_audit_override 2026-08-28 产品召回主研究领域单源足矣)；2026-08-29 fang2025 POM 新增 3 变体：R4 内生调节变量 2SLS fitted-value 交互、R7 排他性理论论证+安慰剂回归双重辩护、R7 控制变量内生性三步递进辩护（剔除复制→保守性论证→滞后 IV；变体 14：选择修正项不显著 + Hausman 显著的双信号诚实收束（null 陈述→转折→存在性结论→精度谦逊；自选择诊断收尾；gulati2005-adaptation-vertical，VERIFIED，switching-regression 族暂落 IV-2SLS 族）；变体 15：互为因果双内生 3SLS 主系统导航——反向因果让步开框+Hausman 预检+系统表导航+OLS 基线例外（gulati_sytch2007，VERIFIED；vs 变体3/4 单向内生修正）；变体 16：恰好识别下工具双条件散文审计（跨规格相关性+无非对应直接效应）+过度识别不可检验诚实边界（gulati_sytch2007，VERIFIED；vs 变体11 量化暴露/变体12 Cragg-Donald——按识别结构互补）；变体 17：3SLS 无直接稳健/聚类 SE 调整时的按簇 bootstrap 推断替代——BP 定位+约束声明+簇感知重抽+qualitatively unchanged（gulati_sytch2007，VERIFIED）） |
...
> ⚠️ **历史快照声明**：以下 ✅ 批次日志为历史快照，数字（如各批"总变体数"）以当期批记录为准，不反映现行计数；现行真值 = 各 corpus 文件 `### 变体 N` 标题 grep 计数 + INDEX 结果类型索引表。
