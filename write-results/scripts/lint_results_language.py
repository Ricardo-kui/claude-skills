#!/usr/bin/env python3
"""Check a Results manuscript against active deterministic language locks."""
...
DEFAULT_END_HEADING = r"^##\s+生成后自检记录"
...
def context_matches(record: dict[str, Any], args: argparse.Namespace) -> bool:
    ...
    if scope == "section":
        if not args.section or normalized(record.get("section")) != normalized(args.section):
            return False
    elif scope == "estimator":
        if not args.estimator or normalized(record.get("estimator")) != normalized(args.estimator):
            return False
    else:
        return False
...
def applicable_records(registry: dict[str, Any], args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    superseded = {
        ...
        if isinstance(item, str) and item.startswith("wrf_")
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("manuscript", help="Results Markdown path, or - for stdin")
...
def main() -> int:
    ...
    if violations:
        print("write-results language lint FAILED")
    ...
    print("write-results language lint PASSED")
