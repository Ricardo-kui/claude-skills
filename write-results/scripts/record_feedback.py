#!/usr/bin/env python3
"""Record or inspect user feedback for the write-results skill."""
...
VALID_SCOPES = {"skill", "project", "section", "estimator"}
VALID_CATEGORIES = {
    "source_intake",
    "section_order",
    "hypothesis_order",
...
def prepare_record(raw: dict[str, Any], default_date: str) -> dict[str, Any]:
    record = {
        ...
        "estimator": normalize(raw.get("estimator")),
    ...
    if record["scope"] == "estimator" and not record["estimator"]:
        raise ValueError("estimator is required when scope=estimator")
...
def main() -> int:
    ...
    registry["records"].sort(key=lambda item: (item.get("scope", ""), item.get("project", ""), item.get("category", ""), item.get("id", "")))
    if not args.dry_run:
