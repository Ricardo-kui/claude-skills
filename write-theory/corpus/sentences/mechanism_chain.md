...
| **双轨 / 对立 / 竞争** | 双轨并行机制链（Track A/B）(L99) · 辩证对立双路径机制（habel2016）(L136) · 对称分组双路径调节机制 (L259) · 竞争机制链（路径 A vs B）(L309) · 辩证对立型机制推演（kalaignanam2017）(L336) · 对称反向双轨机制链 (L1256) · 双原因+三方行为者对比型（weng_yang2024）(L1623) · 双重印记对立通道+持续性差异（qiao2026）(L1530) |
...
### 句式 D：透镜解码混合信号与诊断性衰减（Frame Decoding of a Mixed Signal + Diagnosticity Dampening）

<!-- pattern_id: frame_decoding_mixed_signal; build_type: 跨类型（句式级）; source_papers: ["Pfarrer_Pollock_Rindova_2010_AMJ"]; confidence: medium; sentence_position: why_chain_reasoning -->

**句位**: Reasoning 拍——解释"同一刺激在不同构念框架下为何解码出不同信息"的机制句。

**句式骨架**:
```
[混合信号解码] A [stimulus] generated by a [frame-A] firm therefore conveys both
[positive] and [negative] information—[reading 1: the firm has done X well], but
[reading 2: it has also violated the expected pattern].

[严格正面解码对照] In contrast, for [frame-B] firms [the same stimulus] is likely to be
seen as strictly [positive] information. Further, [the stimulus] is an [affectively
congruent] stimulus whose effect is likely to be accentuated by the [frame-B]
([citations]).

[诊断性衰减缓冲] [The frame] reduce[s] the "diagnosticity" of [negative stimulus],
leading [actors] to give them lesser weight than they would absent the [frame] about
[the valued dimension].
```

**变体**:
- "conveys both positive and negative information—[X], but [Y]" — 破折号引导双读数
- "some degree of [variability] is expected from [frame-B] firms, from which [actors] may not only tolerate, but even anticipate, [variable outcomes]" — 递进让步（not only...but even）
- "the more [proven] the perception of [ability] is in observers' minds, the less diagnostic [negative information] is likely to be" — "the more...the less" 梯度缓冲句

**为什么有效**: 把"框架如何改变刺激含义"写成显式双读数句，机制不再停留在"框架影响反应"的黑箱；诊断性衰减给出缓冲效应的可检验中介语言。
**注意事项**: 两个 reading 必须都从框架定义推出（框架 A 强调一致性 → 违反一致性即负面信号）；缓冲句要指明被衰减的是哪类信息。
**反模式**: 只说 "the frame buffers the reaction" 不给解码步骤——审稿人会问框架到底改变了刺激的哪层含义。
**原文锚点** (Pfarrer et al. 2010, AMJ):
> "A material positive earnings surprise generated by a high-reputation firm therefore conveys both positive and negative information—the firm has performed well, but it has also behaved inconsistently." ... "they reduce the 'diagnosticity' of negative earnings surprises, leading investors to give them lesser weight than they would absent the high-reputation frame about organizational ability."

<!-- wb:pfarrer_pollock_and_rindova_2010:frame_decoding_mixed_signal -->

### 缺席指标的反事实歧义消解（Absence-of-Index Counterfactual Disambiguation，Fini 2017 型）

**句位**: 机制设置段（指标/信号语义消歧），多在 T1 术语定义之后、T3 机制推演之前。

**适用场景**: 当可观察指标只在正向被披露（拒绝不公开、零值多义）时，在机制推演前先声明指标分布的截断性，再用反事实句穷尽"指标缺席"的两种成因（从未寻求评价 vs 寻求但被拒），防止读者把零值读成单一含义、也为后续把"低指标"作为解释对象的做法划定语义边界。

**句式骨架**:
```
[指标可见性声明] These [indices]—either reported by the [candidates] or publicly available—are typically [positive], as [negative outcomes/rejections] are not disclosed.
[缺席反事实消歧] The counterfactual is an absence of [positive indices]—implying that the [candidate] has either [not applied for valuation in the past], or [has applied but has been rejected].
```

**变体**:
- "The counterfactual is an absence of [positive indices]—implying that ..." — 破折号引出双场景枚举（or 连接），标准形
- "Because [negative outcomes] go unobserved, [low/zero levels of the index] may reflect either [inaction] or [unrevealed failure]." — 前置原因 + 双场景后置，语义等价变体

**原文锚点** (Fini, Jourdan & Perkmann 2017, AMJ):
> "These indices—either reported by the candidates or publicly available—are typically positive as grant applications, and thus rejections are not disclosed. The counterfactual is an absence of positive indices—implying that the candidate has either not applied for valuation in the past, or has applied but has been rejected."

**为什么有效**: 截断分布（selection into visibility）是此类指标机制的最大隐性假设；两句话把"零意味着什么"从审稿人暗箭变成作者先手消歧，且为后续假设（指标水平效应）提供了解释弹性。

**反模式**: 指标两侧都可观察（无截断）时不必用本句式；不要把双场景消歧写成第三种未论证成因。

<!-- wb:fini_jourdan_perkmann_2017_amj:absent_index_counterfactual_disambiguation -->

<!-- wb:fini_2017_social_valuation_across_multiple_audiences_the_int:absent_index_counterfactual_disambiguation -->

## Even-If Fallback Refutation（即使退一步，第二维度仍失效，Castellaneta 2017 型）

**适用场景**: 纯调节/双刃剑估值论文中，削弱型机制面临"买方可通过尽调补救"类反驳时，采用"退两步"反驳：先承认反方救济路径存在，再退到最强反方假设（第一维度完全不受损），然后切换到第二维度证明原预测仍成立。用于机制鲁棒性收尾，防止审稿人用单一救济路径推翻论证。

**骨架**:
```
[反方救济路径] [Information] could still be sourced by [decision maker] during [remedial phase], either [channel 1] or [channel 2].
[第一道让步] However, it is unlikely that [voluntary remedial channel] would increase when [treatment strengthens].
[退到最强反方] Moreover, even in the unlikely case where [decision maker] is not worse off in terms of the [dimension 1] of [information], the [dimension 2] of such information might still decrease, because [second mechanism].
[机制依据] [Second mechanism justification with citation].
[一般化回收] Thus, even if [strongest counterfactual], [actor] would still be less able to [task] when [condition].
```

**关键特征**:
- **维度切换而非重复辩护**: 数量守不住退到质量；不在同一维度上反复争辩
- **"even in the unlikely case where"**: 显式标记反方为低概率强假设，steelman 后再反驳
- **互补信息机制**: 第二维度依赖不同信息源（如关于竞争者的互补信息），与第一维度来源独立，故反驳不冗余
...
**与 Concession-Rebuttal 先导模式的区分**: 先导模式在 trade-off 链之前建立"为什么反方收益不占主导"；本变体在削弱型机制链内部作鲁棒性收尾，让步对象是"机制可被补救"而非"对立收益存在"。

**适用边界**: 仅用于同一机制可分解为两个独立维度（数量/质量、速度/精度）且 treatment 对两维度有不同作用渠道时；单一维度的反驳用普通 Concession-Rebuttal。

<!-- wb:castellaneta_2017_smj_how_does_trade_secret_legal_protection:even_if_fallback_refutation -->
...
<!--
pattern_id: dual_edge_scope_qualification
build_type: 跨类型（句式级；Reasoning 句位——qualify 功能）
...
sentence_position: why_chain_reasoning
-->

**句位**: 机制段内、假设收敛前——主动承认 IV 来源的反向边缘，再收敛正向主效应

**句式骨架**:
```
[双刃声明] [IV source] can both [enable edge: enlarge the opportunity set] and [constrain edge: restrict the opportunity set] for [actors].
[使能边具体化] By influencing [mechanism], [source] can help [actors] [identify/achieve X]. Similarly, [source] [mirror-side effect on counterparties].
[约束边具体化] But [source] may also constrain [actors'] set of choices by [constraining mechanism], and for [non-participants] it provides [no benefit] at all, which may [exclude them from opportunities].
[收敛] These factors can be powerful enabling conditions that enhance the likelihood that [actors] will [behavior]. Thus, H[N].
```

**变体**（同句位措辞候选）:
- "can both restrict and enlarge…" — 双刃动词对（restrict/enlarge）
- "for non-participants it provides no information at all, which may lock them out" — 非参与者零取值注记（把约束边推到构念取值域之外，反向保护主效应）

**为什么有效**: 在正向主效应前主动陈述约束边，把"网络也会锁死你"从审稿人的暗箭变成作者先手（qualify 功能）；非参与者注记把约束边归入构念取值为零的情形，使正主效应在构念定义域内无反例——诚实与可检验性同时达成；镜像侧句（对方也据网络判断本方）同时覆盖了被选机制。

**注意事项**: 双刃声明后必须给出收敛理由（为何使能边承载假设）；若约束边足以颠覆预测方向，应升级为调节假设而非用限定句压住；范围注记针对构念取值域（非参与者），不是未检验情境的挡箭牌。

**反模式**: 双刃声明后无收敛直接跳假设（读者自选方向）；用限定句压住本应成立的负向机制（诚实边界失守）。
...
<!-- wb:gulati_1999_network_location_and_learning_the_influence_of_n:dual_edge_scope_qualification -->
<!--
...
- H3 不能直接读两个系数（一正一负）。技巧：对**负向通道**做 reverse-code 使两条 time×channel interaction 同向，再用 **Wald test** 检验两交互系数是否统计可区分（见 `../write-results/corpus/SEM-moderated-mediation.md` "Reverse-Code + Wald Test"）。
...
5. **T6 收束常用历史格言作 foil**（见 `../write-introduction/corpus/theory-lens/05-maxim-contrast.md` 变体 B）。
...
**语料锚定**: qiao_hiatt_sine2026 (SMJ) — 创始军方伙伴 → 内部 capability imprint（操作流程/安全协议，+ 国际化）+ 外部 identity imprint（军方名称/联想，− 国际化）；H3 capability 比 identity 更持久；H4 民用化空管 democratize 能力底物；H5 军事政变 amplify 身份威胁。配合 `../write-methods/corpus/生存分析.md` 变体 6、`../write-methods/corpus/IV-2SLS.md` 变体 4、`../write-results/corpus/IV-2SLS.md` 变体 4 使用。

---

## 双原因+三方行为者对比型主效应推导（weng_yang2024 型）
<!-- wb:weng_yang2024:dual_reason_three_actor_contrast -->
